...
public class ZCWeb {
	...
	protected void initDialog(){
		...
		dialog = new CustomDialog(Fish.myMainActivity);
	...
	public void pay(int rmb, ZCPayIcallback callback){
		...
		if(isSecondConfirm){
			...
			CustomProgressDialog.instShow(Fish.myMainActivity);
			sendPay(rmb, "");
		}
		...
		try {
			...
			if(resultCode.equals("200000")){
				...
				if(!isSecondConfirm){
					...
					Pay.sendThirdPartyTjLog("payFail", ""+Pay.curPayRmb, "2", "24");//24 :赞成web电信   友盟统计中的支付渠道
				}else{
			...
			dialog.dismiss();
			Toast.makeText(context, "付费失败！", Toast.LENGTH_SHORT).show();
			Pay.sendThirdPartyTjLog("payFail", ""+Pay.curPayRmb, "1", "24");//24 :赞成web电信   友盟统计中的支付渠道
			CustomProgressDialog.instDismiss();
		}
	}
	...
	public void submitCode(String code){
		if(code.equals("")){
			if(!isSecondConfirm){
				...
				CustomProgressDialog.instDismiss();
				Pay.sendThirdPartyTjLog("payFail", ""+Pay.curPayRmb, "3", "24");//24 :赞成web电信   友盟统计中的支付渠道
			}else{
    	...
    	try {
			...
			// TODO Auto-generated catch block
			Toast.makeText(context, "付费失败！", Toast.LENGTH_SHORT).show();
			Pay.sendThirdPartyTjLog("payFail", ""+Pay.curPayRmb, "4", "24");//24 :赞成web电信   友盟统计中的支付渠道
			e.printStackTrace();
		}
